...
阈值自动调整问题
    再像素平均值合格后，用边缘扫描得到数列长度来进一步调节时，对比前后图片的像素平均值差异，判断加或者减。
    此版本阈值自动调节只加不减，一眨眼容易加的达到上限，且捕捉瞳孔太早，导致后面在测量直径时出现文字叠加
